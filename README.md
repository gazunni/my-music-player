...
Every song begins as a memory, a feeling, a moment, an atmosphere. Emotional Lenses transforms music discovery from genre-first browsing into emotionally guided exploration — the listener enters through emotional space first.
...
| `gx9k-panel.html` | Admin panel (key-gated, all API routes also key-gated) |
...
The key is stored as a Cloudflare Worker secret `ADMIN_KEY` — never in code. As of v2.8.6, every admin API route (not just the panel page) requires this key via an `X-Admin-Key` header.
...
Worker injects Open Graph meta tags for social preview cards (album cover, title, description). Title/description text is HTML-escaped before injection (v2.8.6).
...
| v2.8.6 | Admin API routes require key on every call, not just panel page load; OG tag XSS fix; `ReferenceError` fix on album edit |

See `generify-music-handoff.md` for full session-by-session detail.
